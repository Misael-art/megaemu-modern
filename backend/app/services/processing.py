...
class ImportService:
        ...
        try:
            ...
            for i, file_path in enumerate(rom_files):
                try:
                    ...
                    task.status_message = f"Processando: {file_path.name}"
                    await db.commit()
                    ...
                    # Importa ROM
                    rom_data = await self._create_rom_data(
                        file_path,
                        system_id,
                        auto_verify,
                        auto_extract
    ...
    async def _handle_dat_import(
        ...
        try:
            ...
            total_games = len(dat_data.get("games", []))
            ...
            for i, game_data in enumerate(dat_data["games"]):
                try:
                    # Atualiza progresso
                    task.progress = int((i / total_games) * 100)
                    task.current_step = i + 1
                    task.status_message = f"Processando: {game_data.get('name', 'Unknown')}"
                    await db.commit()
                    
                    # Importa jogo e ROMs
                    await self._import_game_from_dat(db, game_data, system_id)
                    imported_count += 1
    ...
    async def _create_rom_data(
        ...
        # Calcula hashes
        crc32 = await calculate_file_hash(file_path, "crc32")
        md5 = await calculate_file_hash(file_path, "md5")
        sha1 = await calculate_file_hash(file_path, "sha1")
        ...
        if is_archive_file(file_path):
            ...
            try:
                ...
                print(f"[LOG] Tamanho descomprimido calculado para {file_path}: {uncompressed_size} bytes")
            except Exception as e:
                print(f"[ERROR] Falha ao calcular tamanho descomprimido para {file_path}: {str(e)}")
                uncompressed_size = None
    ...
    async def _parse_dat_file(self, dat_file_path: Path) -> Dict[str, Any]:
        """Faz parse de arquivo DAT/XML.
        
        Args:
            dat_file_path: Caminho do arquivo DAT
            
        Returns:
            Dados parseados do DAT
        """
        ...
        try:
            ...
            for game in root.findall('game'):
                game_data = {
                    'name': game.get('name'),
                    # Adicione mais campos conforme estrutura DAT
                }
                games.append(game_data)
            print(f"[LOG] Parse de DAT concluído: {len(games)} jogos encontrados")
            return {"games": games}
        except Exception as e:
            print(f"[ERROR] Falha no parse de DAT {dat_file_path}: {str(e)}")
            return {"games": []}
...
class VerificationService:
    ...
    def __init__(self, rom_service: ROMService):
        self.rom_service = rom_service
    ...
    async def verify_roms_batch(
        ...
        *,
        ...
        Args:
            db: Sessão do banco de dados
            rom_ids: IDs das ROMs
            user_id: ID do usuário
            
        Returns:
            Tarefa de verificação criada
        """
        task_data = TaskCreate(
        ...
        return await self.task_service.create_task(db, task_in=task_data)
